...
class QwenLLM:
    PROMPT_VERSION = "qwen_local_transformers_v3_feature_cache_compact"
    ...
    def __init__(
        ...
        self.verbose = bool(int(os.getenv("QWEN_LLM_VERBOSE", "0"))) if verbose is None else bool(verbose)
...
"""
    ...
    def _build_cache_key(self, has_key, prev_bfs, curr_bfs, delta_distance, move_type, entered_dead_end, prev_degree, curr_degree):
        return (
            ...
            f"range:{self.min_reward}:{self.max_reward}|key:{has_key}|move:{move_type}|"
            f"prev_bfs:{prev_bfs}|curr_bfs:{curr_bfs}|delta:{delta_distance}|"
            f"dead_end:{entered_dead_end}|prev_deg:{prev_degree}|curr_deg:{curr_degree}"
        )
    ...
    def get_reward_range(self, current_info, prev_info):
        ...
        try:
            ...
            result = self._generate_json_response(self.system_prompt, user_prompt)
