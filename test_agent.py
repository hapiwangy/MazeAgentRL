...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="dataset/test.json", help="Path to the maze dataset used for evaluation.")
    parser.add_argument("--maze_size", type=int, default=None, help="Override maze size filter. Defaults to the checkpoint metadata.")
    parser.add_argument("--maze_id", type=str, default=None, help="Optional maze id to evaluate. If omitted, a random maze is chosen from the dataset.")
    parser.add_argument("--max_steps", type=int, default=None, help="Override the maximum steps. Defaults to the checkpoint metadata.")
    parser.add_argument("--deterministic", action="store_true", help="Use argmax action selection instead of sampling.")
    parser.add_argument("--save_gif", action="store_true", help="Save an evaluation trajectory GIF.")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducible evaluation. Defaults to checkpoint seed.")
    parser.add_argument("--run_name", type=str, default="eval", help="Optional tag to distinguish evaluation outputs.")
    ...
    env = MazeEnv(maze_map=current_maze_data["grid"], max_steps=max_steps, include_text_info=False)
    obs, info = env.reset()
    agent.reset_memory()

    done = False
    total_reward = 0.0
    positions = [info["agent_pos"]] if args.save_gif else None
    ...
    while not done:
        ...
        if positions is not None:
            ...
            if info.get("picked_key_this_step") and key_pick_frame_idx is None:
    ...
    result_filename = os.path.join(eval_dir, f"{run_id}_maze{current_maze_data['id']}_seed{seed}.csv")
    ...
    if args.save_gif and positions is not None:
        gif_name = (
            f"{args.run_name}_{algo}_maze{current_maze_data['id']}_"
            f"{current_maze_data['size']}x{current_maze_data['size']}_seed{seed}.gif"
        )
        save_trajectory_gif(
            reconstruct_episode_frames(current_maze_data["grid"], positions, key_pick_frame_idx),
            current_maze_data["id"],
            filename=gif_name,
            fps=10,
            output_dir=gif_dir,
