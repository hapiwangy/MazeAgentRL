...
class A2CAgent:
    ...
    def select_action(self, obs, has_key):
        ...
        logits, state_value, self.hidden_state = self.network(obs_tensor, has_key_tensor, self.hidden_state)
        distribution = Categorical(logits=logits)
        action = distribution.sample()
        log_prob = distribution.log_prob(action)
        return action.item(), log_prob, state_value.view(1), distribution.entropy()

    def act(self, obs, has_key, deterministic=False):
        obs_tensor, has_key_tensor = self._prepare_inputs(obs, has_key)
        logits, _, self.hidden_state = self.network(obs_tensor, has_key_tensor, self.hidden_state)
